...
function posterPane() {
    var div = $("<div>", {"id": "posters", "class": "probootstrap-text probootstrap-animate tab-pane", "style": "width:100%;margin-right:0;"}).append(
        $("<h3>").text(
            "Poster session (" + posterData.date + ", " + posterData.startTime + "-" + posterData.endTime + ")"
        )
    ).append(
        $("<br>")
    );
    ...
    var list = $("<ul>");
    posterData.papers.forEach((paper) => {
        var paperInfo = papersData[paper];
        list.append(
            $("<li>").append(
                $("<a>", {"href": paperInfo.weburl, "style":"display:block;"}).append(
