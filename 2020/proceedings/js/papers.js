	...
	if (search != null) {
		...
		authors.forEach((author, i) => {
			if (i != 0 && i == authors.length - 1) {
				authors_html.append(document.createTextNode(" and "));
			}
			else if (i != 0) {
		...
		if (paperInfo.license === undefined || paperInfo.license == null || paperInfo.license == "NONE" || paperInfo.license=="NO PUBLISH") {
			...
			if (paperInfo.license == "CC BY ND 4.0") {
				$("#licenseND").show();
			} else if (paperInfo.license == "CC BY-NC-SA") {
				$("#licenseNCSA").show();
			} else if (paperInfo.license == "CC BY-NC-ND 3.0") {
				$("#licenseNCND").show();
			}
		}
